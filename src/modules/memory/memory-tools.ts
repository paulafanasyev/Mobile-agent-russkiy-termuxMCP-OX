...
export function buildMemorySystemPrompt(
  ...
  if (!memory || !memory.enabled || memory.archivedAt) {
    return input.canWrite
      ? ["Memory is enabled, but the memory document is empty.", MEMORY_GUIDANCE].join(
          "\n",
        )
      : "Memory is enabled, but the memory document is empty.";
  }
...
export function createMemoryTools(input: {
  ...
  const tools = {
    writeMemory: tool({
      description:
        "Replace the complete persistent memory document with revised Markdown. Preserve existing durable information unless it is outdated or the user asks to forget it. Never store transcripts, assistant output, temporary tasks, tool results, secrets, or inferred information.",
    ...
    forgetMemory: tool({
      description:
        "Delete the entire persistent memory document only when the user explicitly asks to forget all saved memory. To forget one fact, use writeMemory with the complete revised document instead.",
      inputSchema: z.object({
        reason: z.string().optional(),
      }),
